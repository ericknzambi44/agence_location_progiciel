"""
ViewSet pour le module RH.
Gère les employés et les pointages avec filtrage par agence via AgenceMixin.
"""
...
from config.mixins import AgenceMixin  # <-- import du mixin
...
class RHViewSet(AgenceMixin, viewsets.ViewSet):  # <-- héritage du mixin
    ...
    def list(self, request):
        ...
        """
        try:
            agence_id = self.get_agence_id()  # <-- appel au mixin
    ...
    def create(self, request):
        """
        Embaucher un employé (l'agence_id est automatiquement assigné).
        """
        try:
    ...
    # --- ENREGISTRER POINTAGE : POST /rh/pointages/ ---
    @action(detail=False, methods=['post'], url_path='pointages')
    def enregistrer_pointage(self, request):
        """
        Enregistre un pointage (entrée ou sortie) pour un employé.
        Vérifie que l'employé appartient à l'agence.
        """
        try:
            agence_id = self.get_agence_id()
        except PermissionDenied as e:
        ...
        # Vérifier que l'employé appartient à l'agence
        employe = self.employe_repo.get(data['employe_id'], agence_id=agence_id)
        if not employe:
            return Response({"error": "Employé non trouvé ou non autorisé"}, status=status.HTTP_404_NOT_FOUND)
    ...
    # --- CONSULTER POINTAGES : GET /rh/{id}/pointages/{date}/ ---
    ...
    def consulter_pointages(self, request, pk=None, date_str=None):
        """
        Consulte les pointages d'un employé à une date donnée.
        Vérifie que l'employé appartient à l'agence.
        """
        try:
            agence_id = self.get_agence_id()
        ...
        # Vérifier que l'employé appartient à l'agence
        employe = self.employe_repo.get(UUID(pk), agence_id=agence_id)
        if not employe:
            return Response({"error": "Employé non trouvé ou non autorisé"}, status=status.HTTP_404_NOT_FOUND)
